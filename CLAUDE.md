...
10. **Iterative cooperative routing (GNN ↔ A* feedback loop)** — route in chunks rather than one net at a time to completion. Each chunk: GNN plans zones + net order → A* routes that chunk → actual congestion/failures fed back to GNN → GNN replans zones for next chunk → repeat. This is PathFinder-style negotiated congestion but with a learned planner instead of a fixed cost-inflation rule. Key properties: (a) A* still guarantees no shorts within each chunk, (b) GNN learns to correct its own planning mistakes from observed A* outcomes, (c) generalises to boards GNN has never seen because replanning is driven by real congestion data. No published system combines a learned replanner with a correct executor in this feedback loop — this is the core research contribution.
