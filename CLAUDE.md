...
8. **GNN for net ordering + strategy selection** — input: netlist graph + placement, output: strategy tag per net + routing order. First ML milestone — validates that learned planning improves A* completion rate. See DECISIONS.md option B.
9. **GNN for auto-placement** — predict component positions from netlist graph. Training data: KiCad boards with coordinates. Requires routeability predictor (item 12) first. See DECISIONS.md option C.
10. **Iterative cooperative routing (GNN ↔ A* feedback loop)** — route in chunks rather than one net at a time. Each chunk: GNN plans zones + net order → A* routes that chunk → actual congestion/failures fed back to GNN → GNN replans for next chunk → repeat. Chunk size is adaptive (trigger on congestion threshold, not fixed N). Includes rip-up decisions guided by temporal attention over chunk history. Key properties: (a) A* still guarantees no shorts, (b) GNN learns to correct its own planning mistakes from observed outcomes, (c) no published system combines a learned replanner with a correct executor in this loop — core research contribution. See DECISIONS.md option D.
11. **RL training loop with full DRC reward** — per-fab model (one model per manufacturer profile — strategies are fundamentally different across fabs). Reward structure: hard constraints gate to R=0 (shorts, fatal fab violations, current capacity exceeded, board edge violation, power nets unconnected); soft objectives as weighted product (completion², wire efficiency, via efficiency, DRC score, thermal score). Per-chunk intermediate rewards with discount factor to solve credit assignment across chunks. See DECISIONS.md reward function design.
12. **Routeability predictor** — GNN surrogate that estimates routing completion% from placement without running A*. Prerequisite for placement training (item 9) — provides fast feedback signal during placement search.
13. **Zone decomposition** — GNN predicts coarse tile assignment per net, A* does detailed routing within those constraints. Part of item 10.
