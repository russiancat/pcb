...
def _routed_board():
    ...
    router = Router(grid, nets, rules=rules, max_iterations=2)
    router.route_all()
    return grid, nets, router, rules


def test_no_violations_on_clean_board():
    grid, nets, router, rules = _routed_board()
    violations = run_drc(grid, nets, router, rules)
...
def test_open_net_reported():
    ...
    router = Router(grid, nets, rules=rules)
    # Deliberately do NOT call route_all — net stays open
    violations = run_drc(grid, nets, router, rules)
